...
| `get_drug_competitors(name)` | `CompetitorRawData` (TypedDict) — returns raw `diseases: dict[str, set[str]]` (disease name to competitor drug names) and `drug_indications: list[str]` (approved indication names). Uses `id_to_canonical` to group entries sharing the same `disease_id` under a single canonical disease name. Determines approved indications by filtering `drug.indications` to entries with `max_clinical_stage == "APPROVAL"`. A qualifying disease is retained when the analyzed drug is attached to it, but the analyzed parent molecule and its ChEMBL salt forms are excluded from the competitor set by exact ID. A source-only disease therefore has an empty competitor set. Filters out diseases whose name contains only `BROADENING_BLOCKLIST` words (all-words match). Disease merging, final sorting, and the configured candidate cap are applied by `RetrievalService.get_drug_competitors()`. |
