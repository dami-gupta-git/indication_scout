...
- **services/** — Business logic layer. Implemented: `llm.py` (Anthropic SDK), `embeddings.py` (BioLORD-2023), `disease_helper.py` (LLM disease normalization + MeSH descriptor resolver), `pubmed_query.py` (query building), `retrieval.py` (RAG pipeline), `drug_safety.py` (adverse-event search, drug-wide safety assessment, indication-specific harm), `approval_check.py` (openFDA label + LLM approval extraction), `condition_extraction.py` and `condition_grouping.py` (Europe PMC abstracts → candidate indications; built and tested but not yet called by any agent — see `design_europe_pmc.md`).
...
- When making changes to the cli, flag that corresponding changes need to be made in the React UI.
