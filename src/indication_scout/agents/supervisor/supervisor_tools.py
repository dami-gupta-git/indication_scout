...
def build_supervisor_tools(
    ...
    The clinical trials and mechanism agents are compiled once here and reused across calls.
    ...
    allowlist (lowercase name → (canonical_name, source)). The snapshot reflects closure state at
    call time — intended to be read after the agent loop has finished.
    """

    # Build sub-agents once at construction (except literature — see below)
    ...
    # analyze_literature / analyze_clinical_trials. Keyed: lowercase name → (canonical_name, source)
    allowed_diseases: dict[
        str, tuple[str, Literal["competitor", "mechanism", "both"]]
    ] = {}
    # EFO ID → lowercase name (key into allowed_diseases). Lets the merge dedup mechanism vs
    # competitor entries by ontology ID even when names differ (e.g. "NSCLC" vs the full name).
    allowed_efo_ids: dict[str, str] = {}
    # Raw mechanism candidates as they arrive. analyze_mechanism appends; find_candidates consumes
    # them in merge_and_dedup() after both seed tools finish. Holding the full list (vs merging
    # per-candidate) lets the hierarchical LLM pass see the whole union in one shot.
    mechanism_candidates_buffer: list = []
    # Drug-level mechanism target list, set by analyze_mechanism and read by merge_and_dedup so the
    # hierarchical LLM pass can reason about the drug's MoA when picking survivors.
    ...
    # only buffers raw candidates and sets analyze_mechanism_done (doesn't touch the allowlist).
    ...
    # find_candidates_done is set inside merge_and_dedup so downstream tools only observe the
    # post-dedup allowlist. Both events set in try/finally so a sub-agent crash can't deadlock.
    find_candidates_done = asyncio.Event()
    analyze_mechanism_done = asyncio.Event()

    # Drug-level shared store, populated by sub-agents and surfaced via get_drug_briefing. Keyed by
    # normalized drug name. See supervisor_ideas.md for design rationale.
    drug_facts: dict[str, dict] = {}

    # Holdout-only: artifacts from investigate_top_candidates. That tool calls the analyze_* tools
    # directly (not via the ReAct loop), so their messages don't reach result["messages"]; we stash
    # them here and run_supervisor_agent reads them via get_auto_findings() after the run. Keyed by
    # lowercase canonical name → {"literature": LiteratureOutput, "clinical_trials": ...Output}.
    auto_findings: dict[str, dict] = {}

    # Per-disease sub-agent artifacts written by analyze_literature / analyze_clinical_trials during
    # the run. Used by finalize_supervisor's top-N evidence gate (drop blurbs failing the 0-trials-
    # AND-<N-PMIDs check). Keyed by lowercase canonical name → {"literature": ... | None,
    ...
    def _render_briefing(drug_name: str) -> str:
        ...
        if entry["mechanism_disease_associations"]:
            ...
            for g, d, s in entry["mechanism_disease_associations"][:10]:
                # Hide the placeholder score (not surfaced by MechanismCandidate); show otherwise.
    ...
    async def find_candidates(drug_name: str) -> tuple[str, list[str]]:
        ...
        try:
            ...
            # Always release the seed gate so a failure here can't deadlock the analyze_* tools;
            # they'll see an empty allowlist and reject downstream.
    ...
    async def _find_candidates_impl(drug_name: str) -> tuple[str, list[str]]:
        ...
        # Drug-level intake: populate the shared store with aliases and any FDA-approved
        # indications discovered during the candidate filter.
        ...
        # (e.g. semaglutide × MASH) never reaches the briefing and the supervisor can't reason about
        ...
        # the live path would leak today's approvals into a holdout. Drugs not in the table return
        # [] and approval reasoning is silently disabled for that run (see PLAN_date_before.md).
        ...
        if diseases:
            ...
            if fda_approved:
                ...
                # Record the approved indications in the shared store. Discovered as a side effect
                # of candidate filtering — though dropped from the candidate list, the supervisor
                # needs them to reason about subset/superset relationships (e.g. CML approval makes
                # "myeloid leukemia" ambiguous).
                existing = {
                    ind.lower().strip() for ind in entry["approved_indications"]
                }
        ...
        # Cap applies to competitor entries only. Mechanism-promoted entries are appended after the
        # merge — already small (capped upstream by MECHANISM_TOP_CANDIDATES), and dropping them
        # here would defeat the merge.
        ...
        # vs competitor entries by ontology ID. Names that don't resolve to an EFO (e.g. renamed by
        # the LLM merge) get no entry — the merge falls back to name match in that case.
        ...
        # Wait for analyze_mechanism to populate mechanism_candidates_buffer. Its finally block
        # always sets this gate, so a mechanism crash yields an empty contribution, not a deadlock.
        await analyze_mechanism_done.wait()

        # Run the centralized merge + dedup pipeline. find_candidates_done is set inside
        # merge_and_dedup's finally so downstream readers only ever see the post-dedup allowlist.
        await merge_and_dedup(drug_name)

        # Snapshot the post-dedup allowlist in insertion order: competitor entries first (OT ranked
        # order), then mechanism-only entries (analyze_mechanism's order); hierarchical dedup may
        # have removed entries from either. "both" entries stay in their original competitor slot.
    ...
    async def merge_and_dedup(drug_name: str) -> None:
        ...
        Pipeline (single chokepoint for all candidate-disease dedup):
          1. Exact ID match — drop mechanism candidate if disease_id already in allowed_efo_ids;
             upgrade the matched competitor entry's source to "both".
          2. Exact name match — drop if lowercased name already in allowed_diseases; upgrade to
             "both".
          3. OT name-resolve — resolve unresolved mechanism names to EFO IDs; retry step 1.
          4. Hierarchical LLM pass — over the full merged list, catch super/subtype overlaps the
             exact-match passes miss (UC ⊂ IBD, T2DM ⊂ DM), one survivor per overlap.

        Sets find_candidates_done before returning so downstream readers observe the post-dedup
        allowlist. The find_candidates wrapper's try/finally still sets the gate on exception.
        """
        try:
    ...
    async def _merge_and_dedup_impl(drug_name: str) -> None:
        ...
        # Step 4: hierarchical LLM pass — temporarily disabled while we revisit the motivating case.
        # The dedup was collapsing actionable subtypes (e.g. PCOS, gestational diabetes) into broad
        # parents (metabolic disease) for broadly-acting drugs like metformin. Keep all exact-match
        # candidates until we decide on a hardcoded equivalence-group approach.
    ...
    async def analyze_literature(
        ...
        """
        # Wait for both seed tools to populate the allowlist. Without this, parallel tool calls can
        # hit analyze_literature before the candidates are merged, rejecting legitimate diseases.
        await find_candidates_done.wait()
        await analyze_mechanism_done.wait()

        drug_name = normalize_drug_name(drug_name)
        # Fresh agent per call so literature_tools' closure-scoped store isn't shared across
        # disease invocations.
        ...
        # Build the supervisor-facing summary deterministically from EvidenceSummary so adverse-
        # signal language reaches the supervisor verbatim, with no LLM rewrite in between.
        synth_summary = (
    ...
    async def analyze_clinical_trials(
        ...
        # Drug-level write-through: when the FDA check matches an approved indication, capture it in
        # the briefing so later reasoning sees the approval status. Trial data still flows to the
        ...
        # Normal path: counts come from the exact-count tools (countTotal API). Each scope owns its
        # own count; no cross-scope summing.
        ...
        # safety/efficacy classification is over the top-50 shown terminated trials; if total_count
        # > len(trials) this is a floor.
    ...
    async def _analyze_mechanism_impl(drug_name: str) -> tuple[str, MechanismOutput]:
        ...
        # Buffer raw mechanism candidates for find_candidates to consume in merge_and_dedup() after
        # both seed tools finish. Centralized so the full union can go to a single hierarchical pass.
        ...
        # Drug-level write-through: populate mechanism_targets and mechanism_disease_associations in
        # the shared store. Captured per-MoA so the briefing can show "ABL1 (INHIBITOR), KIT (...)".
        entry = _ensure_drug_entry(drug_name)
        ...
        # surfaced. Raw scores aren't on the candidate model — record the pair without a score.
        ...
        for cand in output.candidates:
            ...
            # only needs "this gene is associated with this disease per OT mechanism evidence" —
            # the briefing renderer hides the placeholder score.
            assocs.append((cand.target_symbol, cand.disease_name, 0.0))
        entry["mechanism_disease_associations"] = assocs

        # Record the mechanism target list so merge_and_dedup() can pass it to the hierarchical LLM
        # pass as context for survivor selection.
    ...
    # candidates like T2DM for semaglutide regardless of prompt — exactly the candidate a holdout
    # tests, so we remove the LLM's ability to skip by auto-investigating the top-10.
    ...
    async def investigate_top_candidates(
        ...
        # dict (not a plain args dict) so .ainvoke() returns a ToolMessage with .artifact populated;
        # a plain dict returns only the content string and loses the typed artifact.
        async def _invest(disease: str) -> tuple[str, dict]:
            ...
            # SupervisorOutput. The ReAct loop doesn't see these messages (invoked directly).
        ...
        # Compact one-line-per-disease summary the LLM can rank against.
        lines = [
    ...
    def _reconstruct_holdout_summary() -> str:
        ...
        becomes one ranked line with literature strength, PMID count, and trial counts pulled from
        the typed sub-agent artifacts. Candidates with zero trials and no usable literature signal
        (same gate finalize_supervisor applies to blurbs) drop into a single "Evidence gate
        exclusions:" footer line.
        """
    ...
    async def finalize_supervisor(
        ...
        for item in blurbs or []:
            ...
            if disease_key not in allowed_diseases:
                ...
                continue
            # Top-N evidence gate: drop candidates with 0 trials AND no usable literature signal —
            # strength="none" OR study_count==0. Strong/moderate/weak literature with ≥1 relevant
            # abstract and 0 trials is a legitimate signal and is kept; the PMID-count fallback
            # applies only when synthesize didn't run. Filtered candidates still appear in
            # disease_findings (per-disease section) — only removed from the top-N ranking + blurbs.
        ...
        if date_before is not None:
            ...
            # APPROVAL RELATIONSHIPS instructions; reconstruction enforces the structured fact-list
        ...
        # Filter the LLM-written summary to drop ranked lines whose disease didn't pass the evidence
        # gate (not in validated). Non-ranked lines (e.g. trailing "Closed signals:") pass through
        # unchanged; survivors are renumbered to keep the rank sequence contiguous. Uses the same
        # regex and longest-substring match as the formatter's _splice_blurbs_into_summary, for
        # consistent disease matching.
        validated_diseases = {e["disease"].lower().strip() for e in validated}
        rank_line = re.compile(r"^\s*(?P<rank>\d+)\.\s+(?P<head>.+?)\s+—\s+(?P<tail>.+)$")
        # Normalize the heading so the report says "signals" regardless of whether the LLM wrote
        # "candidates" / "opportunities" / "indications".
    ...
    def get_merged_allowlist() -> (
        ...
        Returns a copy keyed by lowercase name → (canonical_name, source), where source ∈
        {"competitor", "mechanism", "both"} per which sub-agent surfaced the disease.
        """
    ...
    def get_auto_findings() -> dict[str, dict]:
        ...
        Returns {lowercase_canonical: {"literature": LiteratureOutput, "clinical_trials":
        ClinicalTrialsOutput}}. Empty in non-holdout runs.
        """
    ...
    if date_before is not None:
        # Holdout-only: insert investigate_top_candidates before finalize so the LLM sees it after
        # the seed-phase tools but before terminating.
